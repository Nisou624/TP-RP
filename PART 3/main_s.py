    ...
    def listen_online(self, objects, has_controls):
        ...
        if has_controls:
            ...
            if controls_to_send[pygame.K_LEFT] and not collide_left:
                self.move_left(self.PLAYER_VEL)
            elif controls_to_send[pygame.K_RIGHT] and not collide_right:
                self.move_right(self.PLAYER_VEL)
            elif controls_to_send[pygame.K_UP]:
                if not self.attacking:
                    self.attacking = True
                    self.animation_count = 0
                    self.attack()
            ...
            if controls_received != None:
                ...
                if controls_received[pygame.K_LEFT] and not collide_left:
                    ...
                    self.move_right(self.PLAYER_VEL)
                elif controls_received[pygame.K_UP]:
                    if not self.attacking:
                        self.attacking = True
                        self.animation_count = 0
                        self.attack()
...
def game_online(p1, p2):
    ...
    # Control both players, but only the one corresponding to the client's identity
    if whoami == 1:
        p1.listen_online([terrain, p2], True)
        p2.listen_online([terrain, p1], False)
    else:
...
def listen_server(arg1, arg2):
    ...
    while True:
        if not game_started:
            message = client.recv(1024).decode("utf-8").split(".")[0]
            if message.split(":")[0].strip().lower() == "player":
                whoami = int(message.split(":")[1].strip())
            elif message.split(":")[0].strip().lower() == "pos":
                if whoami == 1:
                    pos = read_pos(message.split(":")[1].strip())
                    player_1 = Player(pos[0], pos[1], 96, 96)
                    print("you are player one and your pos are :", pos)
                else:
                    pos = read_pos(message.split(":")[1].strip())
                    player_2 = Player(pos[0], pos[1], 96, 96)
                    print("you are player two and your pos are: ", pos)
            elif message.split(":")[0].strip().lower() == "last_pos":
                if whoami == 1:
                    pos = read_pos(message.split(":")[1].strip())
                    player_2 = Player(pos[0], pos[1], 96, 96)
                    print("your ennemy (2) pos are: ", pos)
                else:
                    print("you are player 2 and you'll receive the pos of p1")
                    pos = read_pos(message.split(":")[1].strip())
                    player_1 = Player(pos[0], pos[1], 96, 96)
