...
def game_online(p1, p2):
    ...
    if whoami == 1:
        ...
        p2.listen_online([terrain, p1], True, None) # None should be the packet received from the socket
    else:
...
def multi():
    global player_1, player_2, global_state, selected_world
    try:
    ...
    if message.lower() == "welcome":
        ...
        if mode.lower() == "map":
            whoami = 1
            while selected_world == -1:
                draw_selection()

            client.send(str(selected_world).encode("utf-8"))
        else:
            whoami = 2
        Play = False
        while not Play:
            message = client.recv(1024).decode("utf-8")
            draw_text("Waiting for other player...", font, (255,255,255), WIDTH / 2 - 200 / 2, HEIGHT / 2 - 50)
            if message == "pos " + whoami:
                player_1 = Player(read_pos(message)[0], read_pos(message)[1], 96, 96)
            else:
                player_2 = Player(read_pos(message)[0], read_pos(message)[1], 96, 96)
